...
void run()
{
    ...
    double f_A = 2.222e-5, f_B = 2.401, f_tau = 0.1;              // fatigue parameters
    double f_damage_threshold = 1, f_fatigue_limit_ratio = 1.108; // fatigue damage parameters
    ...
    for (Particle<n_layer> *p1 : pt_ass.pt_sys)
    {
        ...
        if (p1->xyz[1] < 19.05 - 5 || p1->xyz[1] > 19.05 + 5)
            p1->type = 3; // undamaged part
        if (p1->xyz[1] > 37.1)
        {
        ...
        ftpt->setParticleProperty(E0, mu0, f_A, f_B, f_tau, f_damage_threshold, f_fatigue_limit_ratio);
    }
    ...
    double tau = 1;                                    // fatigue time mapping parameter
    ...
    int undamage_pt = 3;                               // undamaged particle type
    std::string dumpFile{"DogBone_3DSC_fatigue.dump"}; // output file name
    SolverFatigue<n_layer> solv_fatigue{undamage_pt, pt_ass,
                                        StiffnessMode::Analytical,
                                        SolverMode::CG,
                                        TimeMapMode::Exponential,
